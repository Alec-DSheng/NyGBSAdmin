    ...
    <div class="time-rule">
        <div class="time-day" ref="day" :style="{ left: timeDayX + 'px'}">
            <div :class="['time-minute', minuteActiveClass((n - 1)*minutesPerUnit)]" :style="{width: minutesPerUnit + 'px'}" :title="minuteTitle((n - 1)*minutesPerUnit)"
                v-for="(n,index) in (1440/minutesPerUnit)" :key="index + '-' + n" @click.prevent="clickMinute((n - 1)*minutesPerUnit)"></div>
...
export default {
  ...
  mounted() {
      ...
      $(cursor).on("mousedown", function (e) {
          $(cursor).data("originPageX", e.pageX);
          _this.bMoving = true;
          $(document).on("mousemove", moveCursor).one("mouseup", function () {
          ...
          $(cursor).data("originPageX", touch.pageX);
          _this.bMoving = true;
          $(document).on("touchmove", touchMoveCursor).one("touchend", function () {
      ...
      $(day).on("mousedown", function (e) {
          $(day).data("originPageX", e.pageX);
          _this.bMoving = true;
          $(document).on("mousemove", moveDay).one("mouseup", function () {
          ...
          $(day).data("originPageX", touch.pageX);
          _this.bMoving = true;
          $(document).on("touchmove", touchMoveDay).one("touchend", function () {
  ...
  watch: {
      ...
      timeCursorX: function () {
          this.timeCursorTextMove()
      },
      timeDayX: function () {
          this.timeCursorTextMove()
      }
  ...
  methods: {
     ...
     timeCursorTextMove() {
          ...
          this.timeCursorText = m.format("HH:mm");
      },
  },
  computed: {
      activeMinutes() {
